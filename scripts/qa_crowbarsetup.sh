...
function crowbarctl_upgrade_status
{
    local path=${1:-""}
    local result=${2:-passed}
    crowbarctl upgrade status | grep -q "${path}.*${result}"
}
...
function onadmin_prepare_crowbar_upgrade
{
    if iscloudver 5; then
        ...
        wait_for 300 10 "crowbarctl_upgrade_status steps.prepare.status" "prepare step to finish"
    fi
}
...
{
    if crowbarctl_upgrade_status steps.nodes ; then
        echo "'nodes' step finished successfuly"
        return 0
    fi

    if crowbarctl_upgrade_status "" failed ; then
...
function onadmin_crowbar_nodeupgrade
{
    if iscloudver 6plus ; then
        ...
        if [[ $want_nodesupgrade ]]; then
            ...
            wait_for 300 10 "crowbar_upgrade_status steps.services.status" "services step to finish"
            ...
            wait_for 300 10 "crowbar_upgrade_status steps.backup_openstack.status" "backup openstack step to finish"
