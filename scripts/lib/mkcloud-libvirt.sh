...
function libvirt_onhost_create_vm_config()
{
    ...
    local mac=$(macfunc $number)
        ...
        safely ${scripts_lib_dir}/libvirt/compute-config "$cloud" "$number"\
            ...
            "$(get_nodenumbercontroller)" "$firmware_type" > /tmp/$cloud-node$number.xml
}
...
function libvirt_do_setupnodes()
{
    ...
    for i in $@ ; do
        ...
        if [[ $nodetype = lonely ]] ; then
            local this_disk="$vdisk_dir/${cloud}.node$i"
            lvdisplay "$this_disk" || \
                _lvcreate "${cloud}.node$i" "${lonelynode_hdd_size}" "$cloudvg"
            libvirt_do_onhost_deploy_image "lonely" $(get_lonely_node_dist) "$this_disk"
        fi
